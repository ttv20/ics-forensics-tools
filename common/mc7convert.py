...
def MC7_to_AWL(data):
    ...
    if data:
        ...
        bs_data = BinaryStream(raw_data)
        while True:
            try:
                msg = ''
                op1 = bs_data.readChar()
                if not op1:
                    # print('error! no data')
                    break
                op1_str = op1.hex().upper()
                op1_int = int(op1.hex(), 16)
                op2 = bs_data.readChar()
                if not op2:
                    # print('error! no data')
                    break
                ...
                if op_value in ops:
                    ...
                    if op1_int in commands_16:
                        ...
                        if op1_int == 0x3D:
                            bs_data.readBytes(3)
                            param_length = int((bs_data.readByte() / 2) - 1)
                            for x in range(param_length):
                                param_data = bs_data.readBytes(4)
                                pv = {
                                    0x80: f'P#PE ',
                                    0x81: f'P#E  ',
                                    0x82: f'P#A  ',
                        ...
                        if op1_int == 1:
                            ...
                            commands.append(f'L B#({bs_data.readChar()},{bs_data.readChar()})')
                        elif op2_int == 7:
                            value = bs_data.readUInt16be()
                            ...
                            value = bs_data.readUInt16be()
                            commands.append(f'L D#{value}')  # TODO: convet to date
                        elif op2_int == 0xC:
                            value = bs_data.readUInt16be()
                            commands.append(f'L S5T#{value}MS')  # TODO: convet to Time MS/ H M S
                    elif op1_int == 0x38:
                        if op2_int == 1:
                            value = bs_data.readUInt32be()
                            commands.append(f'L {value}')
                        elif op2_int == 2:
                            value = bs_data.readUInt32be()
                            commands.append(f'L 2#{value}')
                            ...
                            commands.append(
                                f'L B#({bs_data.readChar()},{bs_data.readChar()},{bs_data.readChar()},{bs_data.readChar()})')
                        elif op2_int == 7:
                            value = bs_data.readUInt32be()
                            commands.append(f'L DW#16#{value}')
                        elif op2_int == 9:
                            value = bs_data.readUInt16be()
                            commands.append(f'L T#{value}')  # TODO: convet to date
                        elif op2_int == 0xB:
                            value = bs_data.readUInt16be()
                            commands.append(f'L TOD#{value}MS')  # TODO: convet to Time MS/ H M S
                    elif op1_int == 0x41:
                        value = bs_data.readUInt16be()
                        point_v = int(op2_str[1], 16)
                        op2_v = int(op2_str[0], 16)
                        ...
                        value = bs_data.readUInt16be()
                        if op2_int == 0:
                            commands.append(f'+ {utils.twosComplement_hex(value)}')
                        else:
                            op2_str = op2.hex().upper()
                            point_v = int(op2_str[1], 16)
                            op2_v = int(op2_str[0], 16)
                        ...
                        value = bs_data.readUInt32be()
                        commands.append(f'+ {utils.twosComplement_hex(value)}')  # verify
                    elif op1_int == 0x79:
                        value = bs_data.readUInt16be()
                        op2_str = op2.hex().upper()
                        point_v = int(op2_str[1], 16)
                        ...
                        op2_o = {0x08: 'JOS', 0x18: 'JO', 0x28: 'JP', 0x48: 'JM', 0x58: 'JUO', 0x68: 'JN', 0x78: 'JNBI',
                                 ...
                                 0xF8: 'JC'}
                        commands.append(f'{op2_o[op2_int]} {value}')
            except (KeyError, BinaryStreamParseError):
                break
        return commands
